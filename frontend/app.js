 ...
 * 6. 商業級 8KB Chunked Base64 與連續 3 幀防抖打斷機制 (Debounced Barge-in)
 */
...
// 偵測 Gemini 回傳文字中的文法糾正 (如 "Did you mean: ...?" 或 "Correction: ...")
...
function queueAudioChunk(base64PcmData, sampleRate = DEFAULT_OUTPUT_SAMPLE_RATE) {
    ...
    // 設定 Speak App 語速 (0.8x, 1.0x, 1.2x)
...
function connectWebSocket() {
    ...
    addLog('建立 WebSocket 中繼連線...', 'info');

    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const host = window.location.hostname || 'localhost';
    const port = window.location.port ? window.location.port : '8000';
    const selectedVoice = voiceSelect.value || "Puck";
    const wsUrl = `${protocol}//${host}:${port}/ws?voice=${encodeURIComponent(selectedVoice)}&scenario=${encodeURIComponent(currentScenario)}`;
    ...
    ws.onerror = (error) => { addLog('WebSocket 錯誤', 'error'); };
